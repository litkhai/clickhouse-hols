...
| [local/releases](local/releases/) | **20 per-release feature labs, 25.1 → 26.8** — one directory per release, each with runnable SQL for that version's new features. All 20 have been executed against their own server build; the [index](local/releases/) records which |
...
GitHub secret scanning and **push protection** are enabled on this repository, so a secret is rejected at push time even when the hook is not installed.
...
| [local/releases](local/releases/) | **릴리스별 기능 랩 20개, 25.1 → 26.8** — 릴리스마다 디렉토리 하나, 해당 버전 신기능의 실행 가능한 SQL 포함. 20개 모두 해당 서버 빌드에서 실제 실행 검증했으며, 빌드 번호는 [인덱스](local/releases/)에 표기 |
...
이 저장소는 GitHub secret scanning과 **push protection**이 켜져 있어, 훅이 설치되지 않은 환경에서도 push 시점에 시크릿이 차단됩니다.
