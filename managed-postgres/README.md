...
| [`vector-search/`](vector-search/) ✅ | pgvector, VectorChord and ClickHouse's own vector index over the same million embeddings, compared at matched recall rather than at default settings |
...
| [`vector-search/`](vector-search/) ✅ | 같은 100만 임베딩에 pgvector·VectorChord·ClickHouse 벡터 인덱스를 걸고, 기본 설정이 아니라 **동일 recall**에서 비교 |
