...
|---------|----------|------|----------|------------------|
| [26.9](26.9/) | 2026-09-21 | 4 | 26.9.1.1629 | `LIMIT ... AFTER` / `UNTIL`, `keyValuePairs` text index tokenizer, `ZSTD(3)` as the default codec, `CREATE TOKEN` + scoped `GRANTS` |
...
against, end to end, with zero exceptions. All twenty-one have been. Getting
...
|------|--------|-------|------|-------------|
| [26.9](26.9/) | 2026-09-21 | 4 | 26.9.1.1629 | `LIMIT ... AFTER` / `UNTIL`, `keyValuePairs` 텍스트 인덱스 토크나이저, 기본 코덱 `ZSTD(3)`, `CREATE TOKEN` + 범위 제한 `GRANTS` |
...
빌드입니다. 21개 전부 검증했습니다. 그 과정에서 11개를 고쳐야 했는데, 존재하지
