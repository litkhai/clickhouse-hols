#!/usr/bin/env python3
"""
MV vs RMV 테스트: 빠른 검증 테스트 (5분)
Quick Validation Test (5 minutes) for MV vs RMV Test

5분간 1초당 1,000 rows를 INSERT하여 빠르게 검증
Insert 1,000 rows per second for 5 minutes for quick validation
"""
...
DURATION_MINUTES = 5   # 테스트 지속 시간 (분) - Quick test!
...
def main():
    """메인 실행 함수"""
    print(f"=" * 80)
    print(f"MV vs RMV Test - Quick Validation Test (5 minutes)")
    print(f"=" * 80)
    print(f"Host: {HOST}")
    ...
    try:
        ...
        print(f"📊 Current row count in {TABLE}: {initial_count:,}")
        print()
    ...
    try:
        for batch_num in range(1, TOTAL_BATCHES + 1):
            ...
            try:
                ...
                print(f"❌ Batch {batch_num} insert failed: {type(e).__name__}: {e}")
                if batch_num == 1:
                    # 첫 번째 배치 실패 시 상세 정보 출력
                    import traceback
                    traceback.print_exc()
                    break
                continue
        ...
        try:
            ...
            print(f"✅ Table {TABLE} now has {final_count:,} rows (added {final_count - initial_count:,})")

            # MV 테이블 확인
            result = client.query(f"SELECT count() FROM events_agg_mv")
            mv_count = result.result_rows[0][0]
            print(f"✅ MV table events_agg_mv has {mv_count:,} rows")

            # RMV 테이블 확인
            result = client.query(f"SELECT count() FROM events_agg_rmv")
            rmv_count = result.result_rows[0][0]
            print(f"✅ RMV table events_agg_rmv has {rmv_count:,} rows")
