...
echo "NOTE: TLS is disabled on Kafka broker. NLB will handle SSL termination."
...
services:
  ...
  broker:
    ...
    environment:
      ...
      # KAFKA_ADVERTISED_LISTENERS will be updated by terraform after NLB creation
...
echo ""
echo "================================================================================"
echo "✓✓✓ Confluent Platform with SASL/PLAIN (No TLS on Broker) ✓✓✓"
echo "================================================================================"
echo ""
...
echo "  Direct to EC2:     $PUBLIC_DNS:9092  [SASL_PLAINTEXT - No TLS]"
echo "  Via NLB (SSL):     <NLB_ENDPOINT>:9094  [TLS terminated at NLB]"
echo ""
...
echo ""
echo "NOTE: Kafka broker has NO TLS. NLB provides SSL termination on port 9094."
echo ""
echo "Service URLs:"
...
# Confluent Platform Connection Information (NLB SSL Architecture)

## Architecture Overview
- **Kafka Broker:** SASL_PLAINTEXT (NO TLS) on port 9092
- **NLB:** SSL/TLS termination, forwards to Kafka on port 9092
- **External Access:** Via NLB endpoint on port 9094 with SSL

## Quick Connection Details

### Kafka Bootstrap Servers
- **Direct to EC2 (No Encryption):** $PUBLIC_DNS:9092 [SASL_PLAINTEXT]
- **Via NLB (SSL):** <NLB_ENDPOINT>:9094 [SSL terminated at NLB]
...
### SSL Certificate (for NLB)
The NLB uses a self-signed certificate. Download the NLB certificate from:
- Certificate location: terraform-confluent-aws-nlb-ssl/certs/nlb-certificate.pem
- Use this certificate in \`ssl.ca.location\` for SSL verification via NLB

## Connection Examples

### Python (confluent-kafka) - Via NLB with SSL [RECOMMENDED for External Access]
\`\`\`python
...
config = {
    ...
    'ssl.ca.location': 'nlb-certificate.pem',  # NLB certificate
}

producer = Producer(config)
\`\`\`

### Python (confluent-kafka) - Direct to EC2 (Internal/Testing)
\`\`\`python
from confluent_kafka import Producer
...
### ClickHouse Kafka Engine - Via NLB with SSL
\`\`\`sql
...
### ClickHouse Kafka Engine - Direct to EC2
\`\`\`sql
CREATE TABLE kafka_queue_direct
ENGINE = Kafka()
SETTINGS
...
## Notes
- Kafka broker itself has NO TLS encryption
- SSL/TLS is terminated at the NLB layer
- This architecture is useful for testing NLB SSL termination with Kafka
CONNEOF

# Create Python test script for SASL_PLAINTEXT (port 9092)
cat > /opt/confluent/test_kafka_sasl.py <<PYTEST_EOF
#!/usr/bin/env python3
"""
Test Kafka SASL/PLAINTEXT connection (Direct to EC2, port 9092)
"""
...
# Kafka configuration - SASL_PLAINTEXT on port 9092 (no TLS on broker)
config = {
    'bootstrap.servers': '$PUBLIC_DNS:9092',
    'security.protocol': 'SASL_PLAINTEXT',
    'sasl.mechanism': 'PLAIN',
...
print("================================================================================")
print("Testing Kafka SASL/PLAINTEXT Connection (Direct to EC2)")
print("================================================================================")
print()
print("Configuration:")
...
print()
print("NOTE: This tests direct connection to Kafka broker without TLS.")
print("      For NLB SSL testing, use the NLB endpoint on port 9094.")
print()
